---
name: agent-scope
description: "Bind this working directory's named-workflow scope, the label registered agents and skills suffix their memory identity with: /gutt-pro:agent-scope project|team|individual <name>, or show to see what is in force. The command keeps its compatibility name; the change is applied by the plugin before this text is read."
...
1. **Find the labels already in use**, so a team converges on one instead of each person
   inventing their own. Search the graph for existing named workflow identities and collect the
...
2. **Check what the candidate would collide with.** A registered name is permanent:
   ...
   separated afterwards. If a label you are about to suggest already has workflows whose
...
The label becomes the suffix on every named memory identity registered from here, so a
workflow with base `<workflow-name>` registers as `<workflow-name>--<label>`. Directories
bound to the same label share one identity and one pool of scoped memory; different labels
are isolated. With nothing bound, a workflow derives a label instead — from the git
remote's owner/repo, or the working folder's name when there is no remote.
...
Rebinding does not rename anything. Workflows registered under the old label keep that
