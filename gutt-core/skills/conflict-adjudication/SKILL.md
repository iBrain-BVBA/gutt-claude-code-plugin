...
1. **Scope first — do these even collide?** Different targets, different
   subjects, or different altitude (a general practice and one local variation
   of it), and there is nothing to resolve: **coexist**. Watch the first of
   those — two sides can contradict each other flatly on the same subject and
   still not collide, because each governs something different and neither
   reaches the other's ground. This is the cheapest question and its evidence
   is the most reliable, so ask it first. Scope targets vary in kind and in
   granularity, so normalize before concluding two differ.
