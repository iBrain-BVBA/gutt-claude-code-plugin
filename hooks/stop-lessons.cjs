...
process.stdin.on('end', () => {
  ...
  if (fs.existsSync(stateFile)) {
    ...
    fs.appendFileSync(logFile, `[${timestamp}] Stop hook: Session ${sessionId} already prompted, allowing stop\n`);
    process.exit(0);
  }

  // First stop attempt - create state file and block
  fs.writeFileSync(stateFile, '');
  fs.appendFileSync(logFile, `[${timestamp}] Stop hook: Blocking first stop for session ${sessionId} to prompt lessons\n`);

  // Block Claude from stopping and provide reason
  console.log(JSON.stringify({
    decision: "block",
    reason: "Before completing, consider capturing lessons learned. If significant work was done (decisions, patterns, challenges), use the memory-keeper agent. If trivial work, proceed - you won't be blocked again."
  }));
});
