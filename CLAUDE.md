...
Python 3 stdlib, no dependencies. Outside `gutt-core/` and referenced by no hook.
...
`cases()` and one `evaluate()` per case, and compile every pattern those cases carry.
