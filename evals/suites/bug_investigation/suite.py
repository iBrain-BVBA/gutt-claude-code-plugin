#!/usr/bin/env python3
"""Suite definition: does the skill text turn a bug report into a grounded triage?
...
(signature and symptom searched separately, org scope explicit, no Jira write) and the
report cases on the brief (severity against a stated rubric, a refutable hypothesis, a
resemblance that stays a lead, an absence that names its scope). `corpus.py` owns the
checklists and their rationale; this file only applies them.

A failure is recorded by label (`missing:severity-rubric`, `banned:cause-asserted`), so
the report can say *which* rule a variant loses, not just how often it loses.
"""
import collections
import pathlib
import re
...
DESCRIPTION = "Bug triage: severity with a rubric, refutable hypothesis, cited history"

# Ordinary agent framing: the judge framing would decide the result.
SYSTEM = (
